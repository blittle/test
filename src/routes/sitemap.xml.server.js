...
function shopSitemap(data, baseUrl) {
  const productsData = flattenConnection(data.products).filter(product => product.onlineStoreUrl).map(product => {
  ...
  const collectionsData = flattenConnection(data.collections).filter(collection => collection.onlineStoreUrl).map(collection => {
  ...
  const pagesData = flattenConnection(data.pages).filter(page => page.onlineStoreUrl).map(page => {
